...
# 创建主动脉的KD树（使用镜像并偏移后的顶点）
# aorta_tree = cKDTree(mirrored_aorta_vertices)
...
# 应用掩码到Vmag数据
masked_vmag = Vmag.copy()
masked_vmag[~aorta_mask] = 0

# 创建点云数据
points = np.column_stack((X.flatten(), Y.flatten(), Z.flatten()))
original_values = Vmag.flatten()
...
# 保存数据
np.save(os.path.join(save_dir, 'masked_points.npy'), masked_points)
np.save(os.path.join(save_dir, 'masked_velocities.npy'), masked_velocities)

print(f"分割后的点云数据已保存到 {save_dir} 目录")
